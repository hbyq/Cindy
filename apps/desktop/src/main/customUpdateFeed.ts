...
const SUPPORTED_PLATFORM_KEY = 'win32-x64';

/**
 * Persisted with every staged patch so this fork never mistakes a package from
 * the official updater (or another feed) for one downloaded from its channel.
 */
export const CUSTOM_UPDATE_FEED_ID = 'hbyq-cindy-custom-stable-v1';

export function getCustomUpdatePlatformKey(): string {
  return getPlatformKey();
}

export function isCustomUpdatePlatformSupported(
  platformKey = getCustomUpdatePlatformKey(),
): boolean {
  return platformKey === SUPPORTED_PLATFORM_KEY;
}
...
export async function fetchCustomUpdateManifest(
  ...
  const platformKey = getCustomUpdatePlatformKey();
  // The fork currently publishes only the custom Windows x64 artifact set.
  // Never fall back to the official feed on another platform: doing so could
  // replace a customized installation with an unmodified official package.
  if (!isCustomUpdatePlatformSupported(platformKey)) {
    log.info('Fork app update channel is unavailable for %s', platformKey);
    return null;
  }
  const url = getCustomUpdateManifestUrl(platformKey);
  ...
  return new Promise<Manifest | null>((resolve) => {
    try {
      ...
      request.on('response', (response) => {
        ...
        response.on('end', () => {
          if (settled) return;
          const manifest = parseCustomUpdateManifest(Buffer.concat(bodyChunks).toString('utf8'));
          if (!manifest) {
